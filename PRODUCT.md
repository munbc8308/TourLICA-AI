...
## 2025-11-22 지도 UI/내비게이션 개선
- **풀스크린 지도 뷰**: `/map`이 상단 내비게이션 아래 영역 전체를 점유하도록 재구성하고, Google Maps 기본 줌 컨트롤을 살려 모바일에서도 조작이 가능하도록 했습니다.
- **공통 내비게이션 바**: 상단 바를 `TOURLICA` 로고 텍스트 + 햄버거 버튼 구조로 단순화하고, 우측에는 현재 역할(관광객/통역사·도우미)을 표시해 어떤 모드인지 한눈에 파악할 수 있게 했습니다.
- **햄버거 드로어**: 좌측에서 슬라이드되는 패널을 추가해 `내 정보 → 매칭 히스토리 → 로그아웃` 순으로 노출합니다. 진입 시 `/api/account/profile`, `/api/match/history`를 병렬 호출해 프로필/이력을 로드하고, 로딩·빈 상태 문구도 포함했습니다.
- **부동 정보 패널 토글**: 지도 우측 상단의 설명 패널을 floating 카드로 띄우고, `설명 보기/숨기기` 토글 버튼을 추가해 모바일에서 지도 시야를 가리지 않도록 했습니다. 768px 미만 뷰포트에서는 기본적으로 접힌 상태로 시작합니다.
- **내 위치 이동 버튼**: 지도 우하단 floating 버튼에서 `내 위치로 이동`을 제공해 지오로케이션 감지가 실패해도 기본 좌표(서울 시청)나 최근 좌표로 즉시 되돌릴 수 있습니다.
- **대기 파형 및 경로 시각화**: 관광객이 매칭을 기다릴 때 지도 전체에 ripple 애니메이션을 overlay 하고, Polyline/마커가 통역사·도우미 이동 경로와 현재 위치를 표현해 양측 모두 이동 현황을 공유합니다.

## 2025-11-23 관리자 대시보드 및 안정성 개선
- **관리자 전용 UI**: `/admin` 경로에 관리자 대시보드를 구축하여 전체 사용자 및 매칭 현황을 한눈에 파악할 수 있게 했습니다. 일반 사용자는 접근이 차단되며, 관리자 계정으로 로그인 시 자동으로 대시보드로 리다이렉트됩니다.
- **사용자 관리**: 전체 가입자 목록을 조회하고, 특정 사용자의 계정 활성 상태(`is_active`)를 제어하거나 관리자 메모(`admin_memo`)를 남길 수 있는 기능을 추가했습니다.
- **매칭 관리**: 모든 매칭 이력을 조회하고, 각 매칭의 상세 정보(참여자, 이동 경로 등)를 확인하며, 필요 시 만남 상태(`meeting_status`)를 강제로 변경할 수 있는 기능을 구현했습니다.
- **DB 스키마 최적화**: `match_movements` 테이블의 역할 제약 조건을 완화하여 다양한 역할의 이동 경로 기록을 지원하도록 개선했습니다.
- **실시간 성능 최적화**: 지도 화면에서의 이동 경로 업데이트 주기를 조정(3초 → 10초)하고, 만남 확인 시 캐시된 위치 정보를 활용하여 UI 반응 속도를 획기적으로 개선했습니다.또한, 만남 상태 변경 시 Kafka 이벤트를 발행하여 상대방 화면이 자동으로 갱신되도록 수정했습니다.

## 2025-11-23 국제화(i18n) 및 UI 개선
- **다국어 지원 구현**: `next-intl` 라이브러리를 통합하여 한국어, 영어, 중국어, 일본어 4개 언어를 지원합니다. 사용자 계정 국가 정보를 기반으로 자동으로 언어를 선택하며, 모든 주요 페이지(로그인, 회원가입, 지도, 관리자 대시보드)에 번역을 적용했습니다.
- **실시간 언어 전환기**: 화면 우측 상단에 국기 기반 언어 선택 드롭다운을 추가하여 사용자가 실시간으로 언어를 변경할 수 있습니다. 선택한 언어는 `sessionStorage`에 저장되어 페이지 새로고침 후에도 유지됩니다.
- **번역 API 엔드포인트**: `/api/messages` 엔드포인트를 생성하여 클라이언트가 동적으로 번역 파일을 가져올 수 있도록 했습니다. 이를 통해 빌드 시 발생하던 `ENVIRONMENT_FALLBACK` 오류를 해결했습니다.
- **지도 마커 정보창**: Google Maps 마커 클릭 시 사용자 정보를 표시하는 `InfoWindow`를 추가했습니다. 내 위치 마커는 이름과 역할을, 관광객/전문가 마커는 이름, 역할, 매칭 시간 등의 상세 정보를 표시합니다.
- **만남 확인 버튼 디자인**: 통역사/도우미가 만남 확인을 요청했을 때 관광객 측에 표시되는 확인 버튼에 그린 그라디언트 디자인과 호버 효과를 적용하여 UI를 개선했습니다.
- **빌드 최적화**: `I18nProvider`에 `timeZone` 설정을 추가하여 `next-intl` 관련 경고를 제거했습니다. 모든 페이지가 깔끔하게 빌드되도록 최적화를 완료했습니다.
